...
public class ViewControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int to = Integer.parseInt(request.getParameter("requestTo"));
        int isToday = Integer.parseInt(request.getParameter("today"));
        
        List<Request> listRequest;
        DAO dao = new DAO();
        List<Department> listDepartment = dao.getAllDepartment();
        if (isToday == 1) {
            listRequest = dao.getViewRequestDepartmentToday(to);
        }
        else {
            listRequest = dao.getViewRequestDepartmentPast(to);
        }
